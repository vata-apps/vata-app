...
function TreeSelectPage() {
  ...
  const { data: treesList = [], isLoading, error } = useQuery({
    queryKey: ['trees'],
    queryFn: () => trees.list(),
  });
  ...
  if (error) {
    return (
      <div style={{ padding: "20px" }}>
        <h1>Vata Genealogy</h1>
        <div style={{ color: "red", padding: "10px", backgroundColor: "#ffe6e6", marginBottom: "20px" }}>
          <h3>Error loading trees:</h3>
          <p><strong>Message:</strong> {error.message}</p>
          <p><strong>Stack:</strong></p>
          <pre style={{ fontSize: "12px", overflow: "auto" }}>{error.stack}</pre>
          <p><strong>Full error:</strong></p>
          <pre style={{ fontSize: "12px", overflow: "auto" }}>{JSON.stringify(error, null, 2)}</pre>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      {createTreeMutation.error && (
        <div style={{ padding: "10px", backgroundColor: "#ffe6e6", color: "#d00", marginBottom: "20px" }}>
          ...
          <div>
            {treesList.map((tree) => (
              <div key={tree.name} style={{ 
                padding: "15px", 
                border: tree.fileExists ? "1px solid #ccc" : "1px solid #f44336", 
                marginBottom: "10px",
                backgroundColor: tree.fileExists ? "white" : "#ffe6e6"
              }}>
                <h3>{tree.name}</h3>
                {tree.description && <p><strong>Description:</strong> {tree.description}</p>}
                <p>Path: {tree.path}</p>
                ...
                >
                  {openTreeMutation.isPending ? "Opening..." : tree.fileExists ? "Open Tree" : "Unavailable"}
                </button>
