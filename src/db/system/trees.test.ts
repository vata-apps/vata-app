...
async function seed(data: { name: string; filename: string; description?: string }) {
...
describe('getAllTrees', () => {
  ...
  it('returns all created trees', async () => {
    await seed({ name: 'Smith Family', filename: 'smith.db' });
    await seed({ name: 'Jones Family', filename: 'jones.db' });
    const trees = await getAllTrees();
    expect(trees).toHaveLength(2);
  });

  it('lists the most recently opened tree first', async () => {
    const oldId = await seed({ name: 'Old Tree', filename: 'old.db' });
    const recentId = await seed({ name: 'Recent Tree', filename: 'recent.db' });
  ...
  it('lists never-opened trees after opened trees', async () => {
    const openedId = await seed({ name: 'Opened', filename: 'opened.db' });
    await seed({ name: 'Never Opened', filename: 'never.db' });
...
describe('getTreeById', () => {
  it('returns the tree with matching id', async () => {
    const id = await seed({ name: 'Smith Family', filename: 'smith.db' });
  ...
  it('does not return a different tree', async () => {
    await seed({ name: 'Smith', filename: 'smith.db' });
    const otherId = await seed({ name: 'Jones', filename: 'jones.db' });
...
describe('createTree', () => {
  it('makes the new tree retrievable', async () => {
    const id = await seed({ name: 'New Tree', filename: 'new.db' });
    const tree = await getTreeById(id);
    expect(tree).not.toBeNull();
    expect(tree?.name).toBe('New Tree');
    expect(tree?.filename).toBe('new.db');
  });

  it('stores an optional description', async () => {
    const id = await seed({ name: 'Tree', filename: 'tree.db', description: 'My family' });
    const tree = await getTreeById(id);
    expect(tree?.description).toBe('My family');
  });

  it('stores a null description when none is provided', async () => {
    const id = await seed({ name: 'Tree', filename: 'tree.db' });
    const tree = await getTreeById(id);
  ...
  it('throws when creating two trees with the same filename', async () => {
    await seed({ name: 'First', filename: 'same.db' });
    await expect(seed({ name: 'Second', filename: 'same.db' })).rejects.toThrow();
  });
});

describe('updateTree', () => {
  it('changes the tree name', async () => {
    const id = await seed({ name: 'Old Name', filename: 'tree.db' });
  ...
  it('changes the description', async () => {
    const id = await seed({ name: 'Tree', filename: 'tree.db', description: 'Old' });
  ...
  it('can update name and description at the same time', async () => {
    const id = await seed({ name: 'Old', filename: 'tree.db', description: 'Old desc' });
  ...
  it('does not affect other trees', async () => {
    const id1 = await seed({ name: 'Tree 1', filename: 'tree1.db' });
    const id2 = await seed({ name: 'Tree 2', filename: 'tree2.db' });
...
describe('deleteTree', () => {
  it('removes the tree from the list', async () => {
    const id = await seed({ name: 'To Delete', filename: 'delete.db' });
    await deleteTree(id);
    expect(await getTreeById(id)).toBeNull();
  });

  it('does not delete other trees', async () => {
    const id1 = await seed({ name: 'Keep', filename: 'keep.db' });
    const id2 = await seed({ name: 'Delete', filename: 'delete.db' });
...
describe('updateTreeStats', () => {
  ...
  it('does not change the name when updating stats', async () => {
    const id = await seed({ name: 'Keep This Name', filename: 'tree.db' });
