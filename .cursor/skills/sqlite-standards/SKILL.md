...
- **Never use `SELECT *`**. List columns explicitly (see project rule [no-select-star.mdc](.cursor/rules/no-select-star.mdc)).
...
- **Timestamps**: Prefer `datetime('now')` (or `(datetime('now'))`) as column default for consistency with SQLite.
