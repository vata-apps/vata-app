...
- **Path aliases**: Use project aliases for **cross-directory** imports (e.g. `$components/tree-shell` from a route file). Sibling imports within the same directory stay relative (e.g. `import { TreeNav } from './tree-nav'` inside `src/components/`) — that's the prevailing convention in `src/db/trees/`, `src/managers/`, `src/lib/`, etc. Check `tsconfig.json` for the current alias list.
...
- **Shared pitfalls live in AGENTS.md**: the React.Fragment `key` rule, the `import.meta.env.DEV` debug-UI guard, and the no-unused-exports rule are in AGENTS.md "Common Pitfalls" — not repeated here.
