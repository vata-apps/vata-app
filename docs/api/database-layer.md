...
Rules: explicit column lists (never `SELECT *`), parameterized queries (`$1`, `$2`), IDs converted at the boundary. The full pattern and a scaffold template live in AGENTS.md ("DB Layer Pattern") and the `sqlite-standards` skill (§8).
