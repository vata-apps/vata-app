...
**Brand tokens** are the single source of visual truth in [`src/design/theme.css.ts`](../../src/design/theme.css.ts): colors, radii, shadows, fonts, z-indexes, spacing, and type sizes. Raw values live only there; every component reads them through `vars.*`. The identity is **grayscale** — no hue anywhere in the product, every surface/border/text/status value is a pure neutral at a different lightness (see ADR-0005). **IBM Plex Sans** is the UI and body font, **IBM Plex Mono** is for data (dates, IDs, coordinates, counts), and **Spectral** is reserved for lineage moments — a person's name, unsaved drafts, empty states — never for dense UI chrome. Fonts are self-hosted via `@fontsource/*`.
