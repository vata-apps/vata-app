...
Marie benefits from MVP1 (tree management), MVP2 (GEDCOM import), MVP3 (adding people and events), and MVP4 (polished UI). She needs the experience to be intuitive from the start.
...
Robert is the primary stress-test persona. MVP1 (tree management at scale), MVP2 (robust GEDCOM round-trip), MVP3 (efficient CRUD on large datasets), and the performance targets in the architecture are all designed with Robert in mind.
...
Claire is the target persona for MVP5 (sources and citations) and MVP6 (file management). Her needs validate that the data model must support sources, repositories, and file attachments as first-class entities from the schema design onward.
