...
The UI is built directly on [Radix Themes](https://www.radix-ui.com/themes), with brand tokens set on a `<Theme>` provider and no in-house wrapper layer — see the design system in [docs/ui/design-system.md](../ui/design-system.md).
...
- **Shell**: The in-tree shell (`TreeShell`) wraps every route under `/tree/$treeId/...` with a persistent navigation header above a full-width page body; pages own any panels they need (see [Layouts](../ui/layouts.md)).
...
- No React dependency
- Unit testable
- Uses in-app modules for domain-specific logic (GEDCOM, dates); see [GEDCOM 5.5.1 Mapping](../references/gedcom-551-mapping.md)
