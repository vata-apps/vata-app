...
5. **Verify instead of speculating.** Dependencies are installed — run `pnpm build`, `pnpm lint`, or a scoped `pnpm vitest run <file-or-pattern>` when a finding depends on the answer. Never report that something "could not be verified" when you had the tools to check it. Three rules on commands: typecheck with `pnpm build`, not a bare `pnpm exec tsc --noEmit` (`src/routeTree.gen.ts` is gitignored and generated by `vite build`, so typechecking alone fails with unrelated errors here); scope test runs to the files your finding is about, since the bare suite is 772 tests and you may need several checks; and never use `pnpm test`, whose watch mode will hang until your idle timeout kills it.
